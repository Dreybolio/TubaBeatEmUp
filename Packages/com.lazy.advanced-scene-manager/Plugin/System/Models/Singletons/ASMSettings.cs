...
#pragma warning disable CS0612 // Type or member is obsolete
        ...
        void UpdateInputBindings()
        {
            ...
            foreach (var profile in SceneManager.assets.profiles)
            {
                ...
                if (profile.standaloneScenes.m_sceneBindingsOld is null)
                    continue;

                foreach (var kvp in profile.standaloneScenes.m_sceneBindingsOld)
                {

                    if (kvp.Value.buttons is null)
                        continue;

                    var sceneId = kvp.Key;
                    if (!Scene.TryFind(sceneId, out var scene))
                        continue;

                    foreach (var input in kvp.Value.buttons)
                        ArrayUtility.Add(ref scene.m_inputBindings, new InputBinding(input) { interactionType = kvp.Value.interactionType });

                    scene.Save();

                }

                profile.standaloneScenes.m_sceneBindingsOld = null;
                profile.Save();

            }

            foreach (var collection in SceneManager.assets.collections)
            {

                if (collection.m_inputBindingsOld is null || collection.m_inputBindingsOld.buttons is null)
                    continue;

                foreach (var input in collection.m_inputBindingsOld.buttons)
                {
                    ArrayUtility.Add(ref collection.m_inputBindings,
                        new InputBinding(input)
                        {
                            openCollectionAsAdditive = collection.m_inputBindingsOld.openCollectionAsAdditive,
                            interactionType = collection.m_inputBindingsOld.interactionType
                        });
                }

                collection.m_inputBindingsOld = null;
                collection.Save();
...
#pragma warning restore CS0612 // Type or member is obsolete
        ...
        [Header("Scenes")]
        [SerializeField, FormerlySerializedAs("m_blacklist"), Obsolete] internal List<string> m_blacklistOld = new();
        [SerializeField, FormerlySerializedAs("m_whitelist"), Obsolete] internal List<string> m_whitelistOld = new();
